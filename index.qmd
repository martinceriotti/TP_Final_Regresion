...
-   Primario: tener el nivel primera aumenta \$103 respecto de no tener educación. No es significativo (p = 0.074).

-   Secundario: tener el nivel secundario aumenta \$238 respecto de no tener educación. Es Significativo (p \< 0.001).

-   Superior: el nivel superior de educación aumenta \$607 respecto de no tener educación. Altamente significativo (p \< 0.001).

A mayor educación, mayor ingreso.

##### Región:

Se comparan contra el grupo de referencia "Cuyo."
...
El objetivo de la **razón de odds** es comparar chances de éxito entre 2 grupos diferentes. En la lista anterior, el valor indica cuanto más o menos chances de ganar 1000 o más, tiene el grupo, en comparación con la categoría de referencia (la que no aparece en la lista).
...
-   ant_laboralMas de 5 años: tener más de 5 años de antiguedad laboral multiplica por 2.1 las chances de igualar o superar los 1000 de ingreso ocupacional.
...
#| message: false
#| warning: false
predicciones <- predict(mod_glm, newdata = entrenar_log, type = "response")
predicciones_test <- predict(mod_glm, newdata = probar_log, type = "response")

curvaROC <- roc(
    response = entrenar_log$y,
...
# El punto de corte óptimo lo calculás en función de los datos de entrenamiento. Luego, aplicás el valor hallado para clasificar a los datos de prueba y construir la matriz de confusión.
